...
def main():
    ...
    help_message = '''processing.py --input <measurementsfolder> --output <output> --options "option1,option2,.." --model "model1,model2,..." --column "col1,col2,..."
Options include :
...
--'''+ '\n--'.join(list_possible_models)
    ...
    parser.add_argument('--model',type=str,help='List of models')
    ...
    if args.options:
        ...
        if 'default-cols' in options_list:
            default_cols = True
        if 'explain' in options_list:
            explain=True
        if 'all-models' in options_list:
            list_models = list_possible_models
    
    if args.model:
        for mod in model_list:
            if mod in list_possible_models:
