...
def draw_2D(df, condition, method="UMAP", draw=True, savefig=True, prefix_save='', k=5, col_color=[], palette=None,
    ...
    if method=="UMAP":
        reducer = umap.UMAP(n_neighbors=25,min_dist=.75,metric = 'euclidean',n_components=2,random_state=42)
    elif method=="PCA":
        ...
        if colorcoding_condition=="KMeans":
            ...
            for cluster in np.unique(KMeans_labels):
                plt.scatter(embedding[KMeans_labels==cluster,0],
                            ...
                            s=10, alpha=0.7, label=cluster, color=palette[cluster])
            ...
            for condition_name in np.unique(df[colorcoding_condition]):
                plt.scatter(embedding[df[colorcoding_condition]==condition_name,0],
                            ...
                            s=10, label=condition_name, alpha=0.7, color=palette[condition_name])
            ...
            sc = plt.scatter(embedding[:,0],
                        embedding[:,1],
                        s=10, label=colorcoding_condition, alpha=0.7, c=df[colorcoding_condition])
            clb = plt.colorbar(sc)
...
def main():
    ...
    help_message = '''processing.py --input <measurementsfolder> --output <output> --options "option1,option2,.." --figure "figure1,figure2,..." --column "col1,col2,..."
Options include :
...
--'''+ '\n--'.join(list_figs)
    ...
    parser.add_argument('--input',type=str,required=True, help='Path to the measurements folder')
    parser.add_argument('--output',type=str,required=True, help='Path to the output visualization folder')
    parser.add_argument('--options',type=str,help='List of options')
    parser.add_argument('--figure',type=str,help='List of figures')
    parser.add_argument('--column',type=str,help='List of columns')
    ...
    if args.options:
        if 'no-draw' in options_list:
             draw=False
        if 'no-save' in options_list:
            savefig=False
        if 'default-cols' in options_list:
            default_cols = True
        if 'all-figs' in options_list:
