...
print("Checking FIJI plugin configuration----------------------------- OK\n");
	...
	if (error_configuration == 0){
		...
		folder_VE = matches(directory_VE, ".*/");
		if (folder_VE == 0){
			directory_VE = directory_VE+"/";
		}
		gpu_choice = Dialog.getChoice();
		directory_VE2 = Dialog.getString();
		folder_VE2 = matches(directory_VE2, ".*/");
		if (folder_VE2 == 0){
			directory_VE2 = directory_VE2+"/";
		}
									...
									if (list_img.length > 0){
										...
										if (error_cellpose_diameter == 0){
											...
											for (j=0; j<list_img.length; j++) {
												...
												if (mito_size2 == "Depending on Image"){
													if ((isNaN(object_detection_size[j]) == 1) || (object_detection_size[j] == 0)){
														error_cellpose_diameter = 1;
													}
												}
														...
														if ((extension_tmp == 1) || (extension_tmp2 == 1) || (extension_tmp3 == 1) || (extension_tmp4 == 1) || (extension_tmp5 == 1) || (extension_tmp6 == 1)){
															...
															if ((mito_size == "Same size") || ((mito_size == "Variable size") && (mito_size2 == "Depending on Condition"))){
																if ((object_detection_type == "cyto2") || (object_detection_type == "cyto") || (object_detection_type == "cyto3") || (object_detection_type == "nuclei")){
																	run("Cellpose ...", "env_path="+directory_VE+" env_type=conda model="+object_detection_type+" model_path= diameter="+object_detection_size+" ch1=0 ch2=-1 additional_flags="+flags);
																...
																if ((object_detection_type == "cyto2") || (object_detection_type == "cyto") || (object_detection_type == "cyto3") || (object_detection_type == "nuclei")){
																	run("Cellpose ...", "env_path="+directory_VE+" env_type=conda model="+object_detection_type+" model_path= diameter="+object_detection_size[j]+" ch1=0 ch2=-1 additional_flags="+flags);
							...
							for (j=0; j<list_img.length; j++){
								...
								if (num_roi != 0){
									...
									for (a=0;a<num_roi;a++){ 
										...
										if (isNaN(mito_Kurtosis) == 1){
											mito_Kurtosis = "";
										}
										if (isNaN(mito_Skewness) == 1){
											mito_Skewness = "";
										}
										...
										if ((mito_Area > 10) && (isNaN(mito_Kurtosis) == 0) && (isNaN(mito_Skewness) == 0) && (isNaN(mito_Mean_CORR) == 0) && (isNaN(mito_Median_CORR) == 0) && (isNaN(mito_Int_CORR) == 0) && (isNaN(sd_int_CORR) == 0) && (isNaN(mean_var_CORR) == 0) && (isNaN(mito_Major) == 0) && (isNaN(mito_Minor) == 0) && (isNaN(mito_Angle) == 0) && (isNaN(mito_FFT_Area) == 0)){
