...
def histograms(df, condition, savefig=True, prefix_save='', draw=True, kind="density", palette=None, legend=True,
    ...
    for i, col in enumerate([col for col in df_densities.columns if col!=condition]):
        ...
        if kind=="violin":
            sns.violinplot(data=df, y=col, x=condition, palette=palette, ax=axes[i//3, i%3])
...
def train_models(X, y, save_model=True, save_name='', random_state=0):
    ...
    dict_gs = {name:{'pipe':pipe, 
                     'params':params, 
                     'gs': None, 
                     'results': None} for name, pipe, params in zip(['xgb','rf','lr','mlp'],
                                                                    [pipe_xgb, pipe_rf, pipe_lr, pipe_mlp],
                                                                    [params_xgb, params_rf, params_lr, params_mlp])}
...
def draw_confusion_matrixes(dict_gs, display_labels, savefig=True, prefix_save='', normalize="true"):
    ...
    for model_name in dict_gs:
        if not (model_name.startswith("X_") or model_name.startswith("y_")):
            ...
            if savefig: plt.savefig(prefix_save+"/"+model_name+"_confusion_matrix.png", bbox_inches='tight', dpi=300)
            plt.show()
            
def treeshap_beeswarms(model, X, class_names, savefig=True, prefix_save=''):
...
def kernelshap_beeswarms(model, X, class_names, savefig=True, prefix_save=''):
    explainer = shap.KernelExplainer(model, data=X, feature_names=X.columns, output_names=class_names)
    ...
    for i in range(explanation.shape[2]):
        ...
        plt.title(class_names[i])
        if savefig: plt.savefig(prefix_save+"/"+class_names[i]+"_beeswarm.png", bbox_inches='tight', dpi=300)
        plt.show()
    return explanation
...
def main(argv):
    ...
    list_cols = ['Condition_Name', 'Image_Name', 'Mito_ID', 'Mito_Area',
       ...
       'Intensity_SD_percent', 'Mito_MeanInt_CORR', 'Mito_MedianInt_CORR', 'Mito_TotalInt_CORR', 'Intensity_SD_CORR',
       'Intensity_SD_percent_CORR', 'Mito_CentroidX', 'Mito_CentroidY', 'Mito_Circularity', 'Mito_Roundness', 'Mito_Solidity',
