...
print("Checking FIJI plugin configuration----------------------------- OK\n");
	...
	if ((error_input_tree == 0) && (error_output_tree == 0)){
		...
		if ((error_python_file == 0) && (error_cellpose == 0)){
			...
			run("ROI Manager...");
			analyze_choice = newArray("default","custom");
			segmentation_choice = newArray("Use CELLPOSE: DEFAULT settings","Use CELLPOSE: CUSTOM settings","Use your OWN SEGMENTATION maps");
			...
			if (cellpose_choise == "Yes"){
				...
				if (cellpose_choise2 == "Use CELLPOSE: CUSTOM settings"){
					...
					filter_method_tmp = newArray("Restore BLURING images","Restore NOISY images","Enhance segmentation for HIGH RESOLUTIVE images");
					cellpose_model = newArray("GeneralistModel_GM_EM","Generalist-Fish_Model_GM-FISH_EM","SpecialistModel_SM_C-ELEGANS_EM","SpecialistModel_SM_KILLIFISH_EM","SpecialistModel_SM_FLY_EM","SpecialistModel_SM_MOUSE_EM","SpecialistModel_SM_ZEBRAFISH_EM","SpecialistModel_SM_HUMAN_EM","nuclei","cyto","cyto2","cyto3","Custom model");
					Dialog.create("Image analysis : Parameters - Segmentation");
					Dialog.addMessage("1- GENERAL PARAMETERS");
					Dialog.addChoice("Unit of length for mitometrics output:", choice);
					Dialog.addChoice("Crop raw images ?", choice0);
					Dialog.addChoice("IF YES, select cropping method:", choice2);
					Dialog.addMessage("\n2- IMAGE PREPROCESSING");
					Dialog.addChoice("Select method for intensity normalization:", choice3);
					Dialog.addChoice("Use filtering/restoration to improve image quality ?", choice0);
					Dialog.addChoice("IF YES, select filtering method:", filter_method_tmp);
					...
					filter_method = Dialog.getChoice();
					...
					if (mito_size == "Depending on Condition"){
						...
						for (cond=0;cond<list_dataset.length;cond++){
							...
							if (object_detection_size[cond] != 0){
								if ((object_detection_size_max[cond] == 0) || (isNaN(object_detection_size_max[cond]) == 1) || (parseFloat(object_detection_size_max[cond]) <= parseFloat(object_detection_size[cond]))){
									object_detection_size_max[cond] = NaN;
								}
								if ((object_detection_size_min[cond] == 0) || (isNaN(object_detection_size_min[cond]) == 1) || (parseFloat(object_detection_size_min[cond]) >= parseFloat(object_detection_size[cond]))){
									object_detection_size_min[cond] = NaN;
								}
							} else if (object_detection_size[cond] == 0){
								object_detection_size_max[cond] = NaN;
								object_detection_size_min[cond] = NaN;
							}
						}
				...
				} else if (cellpose_choise2 == "Use CELLPOSE: DEFAULT settings"){
				...
				} else if (cellpose_choise2 == "Use your OWN SEGMENTATION maps"){
			...
			if ((prediction_choice == "Yes") || (ml_choice == "Yes")){
				...
				Dialog.addChoice("Mitometrics selection", analyze_choice);
				Dialog.addHelp("https://www.emitometrix.org/tutorials.html");
				Dialog.addMessage("\n\n\n(See Help Button for plugin quick start tutorial)");
				Dialog.show();
			...
			if (prediction_choice == "Yes"){
				...
				Dialog.addChoice("Data DISPLAY settings", analyze_choice);
				Dialog.addHelp("https://www.emitometrix.org/tutorials.html");
				Dialog.addMessage("\n\n\n(See Help Button for plugin quick start tutorial)");
				Dialog.show();
			...
			if (ml_choice == "Yes"){
				...
				Dialog.addChoice("Data PREDICTION settings", analyze_choice);
				Dialog.addHelp("https://www.emitometrix.org/tutorials.html");
				Dialog.addMessage("\n\n\n(See Help Button for plugin quick start tutorial)");
				Dialog.show();
			...
			if ((error_prediction_choice == 0) && (error_morpho_choice == 0) && (error_ml_choice == 0) && (error_cellpose_diameter == 0)){
				...
				if (cellpose_choise == "Yes"){
					...
					if (cellpose_choise2 != "Use your OWN SEGMENTATION maps"){
												...
												while ((img_check == 0) && (img_compt < object_detection_name.length)){
													if (list_img[cha] == object_detection_name[img_compt]){
														...
														if (imagename_tmp == 0){
															...
															if (object_detection_size[cha] != 0){
																if ((object_detection_size_min_tmp[img_compt] == 0) || (isNaN(object_detection_size_min_tmp[img_compt]) == 1) || (parseFloat(object_detection_size_min_tmp[img_compt]) >= parseFloat(object_detection_size_tmp[img_compt]))){
																	object_detection_size_min[cha] = NaN;
																} else {
																	object_detection_size_min[cha] = object_detection_size_min_tmp[img_compt];
																}
																if ((object_detection_size_max_tmp[img_compt] == 0) || (isNaN(object_detection_size_max_tmp[img_compt]) == 1) || (parseFloat(object_detection_size_max_tmp[img_compt]) <= parseFloat(object_detection_size_tmp[img_compt]))){
																	object_detection_size_max[cha] = NaN;
																} else {
																	object_detection_size_max[cha] = object_detection_size_max_tmp[img_compt];
																}
															} else if (object_detection_size[cha] == 0){
																object_detection_size_min[cha] = NaN;
																object_detection_size_max[cha] = NaN;
															}
											...
											if (error_cellpose_diameter == 0){
												...
												if (imagename_tmp == 0){
													...
													if ((extension_tmp == 1) || (extension_tmp2 == 1) || (extension_tmp3 == 1) || (extension_tmp4 == 1) || (extension_tmp5 == 1) || (extension_tmp6 == 1)){
														...
														if (cellpose_choise2 != "Use your OWN SEGMENTATION maps"){
															selectWindow(name_img);
															if ((filter_choice == "Yes") && (filter_method == "Enhance segmentation for HIGH RESOLUTIVE images")){
																run("Gaussian Blur...", "sigma=4");
															}
															
															if ((gpu_choice == "Yes") && (filter_choice == "No")){
																flags = "--use_gpu";
															} else if ((gpu_choice == "No") && (filter_choice == "No")){
																flags ="";
															} else if ((gpu_choice == "Yes") && (filter_choice == "Yes")){
																if (filter_method == "Restore BLURING images"){
																	flags ="--use_gpu, --restore_type, deblur_cyto3";
																} else if (filter_method == "Restore NOISY images"){
																	flags ="--use_gpu, --restore_type, denoise_cyto3";
																}
															} else if ((gpu_choice == "No") && (filter_choice == "Yes")){
																if (filter_method == "Restore BLURING images"){
																	flags ="--restore_type, deblur_cyto3";
																} else if (filter_method == "Restore NOISY images"){
																	flags ="--restore_type, denoise_cyto3";
																}
															}
															...
															if ((isNaN(max_diameter_tmp) == 0) || (isNaN(min_diameter_tmp) == 0)){
																...
																} else if (isNaN(max_diameter_tmp) == 1){
																	num_roi_large = 0;
																}
																
																if (isNaN(min_diameter_tmp) == 0){
																	selectWindow(name_img);
																	if ((object_detection_type == "cyto2") || (object_detection_type == "cyto") || (object_detection_type == "cyto3") || (object_detection_type == "nuclei")){
																...
																} else if (isNaN(min_diameter_tmp) == 1){
																	num_roi_small = 0;
																}
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				// EXTERNAL SEGMENTATION MAPS IMPORT
				//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				if (cellpose_choise == "Yes"){
					...
					if (cellpose_choise2 == "Use your OWN SEGMENTATION maps"){
						...
						print("\nEXTERNAL SEGMENTATION MAP IMPORT-------------------------------- STARTING");
						
						list_dataset = getFileList(dir2+"/Image_output/processed_images/");
						list_dataset = Array.sort(list_dataset);
						
						for (i=0; i<list_dataset.length; i++){
							
							dataset=replace(list_dataset[i], "/", "");
							Dialog.create("Segmentation map Import");
							Dialog.addMessage("Condition "+dataset);
							Dialog.addMessage("\n1- Select folder containing your OWN SEGMENTATION MAPS");
							Dialog.addDirectory("SEGMENTATION directory", "");
							Dialog.addMessage("\n2- Make sure the segmentation maps have the same name as the INPUT images");
							Dialog.addHelp("https://www.emitometrix.org/tutorials.html");
							Dialog.addMessage("\n\n\n(See Help Button for plugin quick start tutorial)");
							Dialog.show();
							dir_own = Dialog.getString();
							folder_dir_own = matches(dir_own, ".*/");
							if (folder_dir_own == 0){
								dir_own = dir_own+"/";
							}
							
							list_dataset_preproc = getFileList(dir2+"/Image_output/processed_images/"+list_dataset[i]);
							list_dataset_preproc = Array.sort(list_dataset_preproc);
							
							list_dataset_ownmaps = getFileList(dir_own);
							list_dataset_ownmaps = Array.sort(list_dataset_ownmaps);
							
							for (j=0; j<list_dataset_ownmaps.length; j++){
							
								imagename_tmp = 0;
								imagename_tmp = matches(list_dataset_ownmaps[j], ".*[]!#$%&'()/*,:;<=>?@^`{|}~].*");
													
								if (imagename_tmp == 0){
									
									extension_tmp = matches(list_dataset_ownmaps[j], ".*.tif.*|.*.TIF.*");
									extension_tmp2 = matches(list_dataset_ownmaps[j], ".*.jpg.*|.*.JPG.*");
									extension_tmp3 = matches(list_dataset_ownmaps[j], ".*.tiff.*|.*.TIFF.*");
									extension_tmp4 = matches(list_dataset_ownmaps[j], ".*.png.*|.*.PNG.*");
									extension_tmp5 = matches(list_dataset_ownmaps[j], ".*.jpeg.*|.*.JPEG.*");
									extension_tmp6 = matches(list_dataset_ownmaps[j], ".*.bmp.*|.*.BMP.*");
									
									if ((extension_tmp == 1) || (extension_tmp2 == 1) || (extension_tmp3 == 1) || (extension_tmp4 == 1) || (extension_tmp5 == 1) || (extension_tmp6 == 1)){
										
										path = dir_own+list_dataset_ownmaps[j];
										name_img=File.getNameWithoutExtension(path);
										name_img=replace(name_img, " ", "");
										name_img=replace(name_img, "+", "");
										name_img=replace(name_img, "-", "_");
										name_img_final=name_img+"_preproc";
										
										keep_map = 0;
										compt_maps = 0;
										while ((keep_map == 0) && (compt_maps < list_dataset_preproc.length)){
											index_tmp2 = indexOf(list_dataset_preproc[compt_maps],".");
											name_img2 = substring(list_dataset_preproc[compt_maps], 0, index_tmp2);
											if (name_img_final == name_img2){
												File.copy(dir_own+list_dataset_ownmaps[j], dir2+"/Image_output/cellpose_mask/"+list_dataset[i]+name_img+"_cp_masks.tif");
												keep_map = 1;
											} else if (name_img_final != name_img2){
												compt_maps = compt_maps + 1;
											}
										}
